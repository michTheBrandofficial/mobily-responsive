...
const DeviceSelectButton: Nixix.FC = (): someView => {
  ...
  return (
    <Popover transformOrigin="bottom-right">
      ...
      <Popover.Content className="tws-right-9 tws-bg-transparent tws-min-w-[280px] tws-rounded-[32px] !tws-min-h-fit tws-h-fit tws-bottom-6 tws-border-none tws-font-Rubik">
        ...
        <div className="tws-pb-5 tws-text-white tws-pl-5 tws-bg-[#030708] tws-flex tws-flex-col tws-border-b tws-border-b-[#1D1D1F] ">
          <For each={devices}>
            {(item) => (
              <Popover.Close
                onClose={(close) => {
                  // close the popover, then do the do
                  close();
                  setTimeout(() => {
                ...
                className="tws-w-full tws-font-normal tws-cursor-pointer tws-py-2.5 tws-flex tws-items-center tws-gap-x-3 tws-border-b tws-border-b-[#1D1D1F] "
              >
                <CheckIcon
                  style={{
                    display: memo(() => (selectedDevice.value === item.value) ? 'block' : 'none', [selectedDevice])
                  }}
                  fill="none"
                  stroke-width={3}
                  width={20}
